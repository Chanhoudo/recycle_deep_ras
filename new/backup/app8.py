...
# 디렉토리 설정
ORIGINAL_IMAGE_DIRECTORY = '/home/aisw/static/original'
...
def generate_label_image(original_image_path):
    ...
    label, confidence, class_probs = predict_image(original_image_path)
...
def generate_graph():
    ...
    _, _, class_probs = predict_image(os.path.join(ORIGINAL_IMAGE_DIRECTORY, ORIGINAL_IMAGE_FILES[current_image_index]))
...
def image_data():
    ...
    return jsonify(
        ...
        graph_img=graph_img,
        label=labels[np.argmax(predict_image(image_path)[2])]
    )
